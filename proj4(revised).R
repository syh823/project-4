...
newt = function(theta,func,grad,hess=NULL,...,tol=1e-8,fscale=1,
  ...
  if(inherits(hess, "NULL") == TRUE) hess_mat = Hfd(theta = theta, grad = grad, eps = eps, ...)
  ...
  while(sum(abs(grad_vec)>tol*abs(original_f+fscale))!=0 & count_maxit < maxit 
    ...
    if(count_max.half > max.half)
      warning("step fails to reduce the objective despite trying", 
  ...
  if(class(try (chol(hess_mat)))[1]=='try-error'){
    return(output) 
    warning("hessian matrix not positive definite")  
  } else {
